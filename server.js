...
function extractSongQuery(text) {
  ...
  /* 剥离开头的量词 */
  s = s.replace(/^(一?首歌|首歌|一?首|一?个|一?些|个|些)/, '');
  s = s.replace(/(的歌|的歌曲|的曲|歌曲|吧|呢|啊|呀|嘛|呗|了|~|～|。|！|!|？|\?)$/g, '');
  s = s.trim();
  /* 无实义词 → 视为随机换歌 */
  if (!s || /^(歌|曲|新|点|别的|其他|其他歌|不同|风格|音乐)$/.test(s)) return '';
  return s.slice(0, 30);
}
...
+ '【必须执行】只要用户表达了「想听某首歌/某歌手/换个歌」的意图'
    ...
    callLLM(messages, (err, reply) => {
      ...
      /* P9m: 保险 —— 用户明确「想听/点歌」时，若模型只给了 RECOMMEND（不播放），
       * 自动升级为 PLAY，避免"给了卡片但歌没换"。 */
      if (!playMarks.length && recMarks.length
          && /想听|我要听|要听|听一下|来首|放首|点一首|点首|唱一首|播一首|放一首|放一?首|换[^,。！？\s]{0,12}(歌|曲|音乐)|帮[我你]?放|给[我你]?放/.test(String(b.text || ''))) {
        playMarks = recMarks.slice();
        recMarks = [];
      }
      /* 执行点歌(自动播放) */
