 ...
 *   1. roomId 形态是 `<32位hex>_<10位时间戳>`，不是纯数字。
 *      用 \d+ 匹配永远失败。
 *   2. 参数被 URL 编码（roomId%3D...），且邀请藏在 user.lastMsg，
 *      不在 msg 字段，必须深扫。
 *
 * 另：inviterId 是「房间发起人」，实测有时就是自己的 uid；
 *     真正给你发私信的人在会话的 fromUserId —— 这里作为 senderUid 单独返回。
